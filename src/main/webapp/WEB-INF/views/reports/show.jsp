...
<c:set var="commDes" value="${ForwardConst.CMD_FOLLOW_DESTROY.getValue()}" />
    ...
    <c:param name="content">
        ...
        <table>
            <tbody>
                ...
                <tr>
                    <th>いいね！数</th>
                    <td><c:out value="${report.goodCount} 件" /></td>
            </tbody>
        </table>
        ...
        <c:choose>
            ...
            <c:when test="${good_count < 1}">
                <a href="<c:url value='?action=${actRep}&command=${commGod}&id=${report.id}' />">この日報にいいね！する</a>
            </c:when>
            <c:otherwise>
            </c:otherwise>
        </c:choose>
        ...
        <c:when test="${follow_count > 0 }">
          <p>
            <a href="<c:url value='?action=${actRep}&command=${commDes}&id=${report.id}' />">この日報の作成者のフォローをやめる</a>
          </p>
        </c:when>
        </c:choose>
        <p>
